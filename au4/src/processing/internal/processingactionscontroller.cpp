...
void ProcessingActionsController::clipDelete()
{
    ...
    if (selectedClipKey.index != nidx) {
        auto clip = project->processingProject()->clipList(selectedClipKey.trackId)[selectedClipKey.index];
        processingInteraction()->removeClip(selectedClipKey, clip.startTime, clip.endTime);
    }
    ...
    for (const auto& track : tracks) {
        ...
        for (const auto& clip: clips) {
            ...
            processingInteraction()->removeClip(clip.key, selectedStartTime, selectedEndTime);
        }
    }
