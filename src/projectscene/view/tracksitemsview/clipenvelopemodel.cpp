...
void ClipEnvelopeModel::setPoint(int index, double tAbs, double value, bool completed)
{
    ...
    // Begin (only once per drag, and restart if index changed)
    ...
    // Optimistic update for smooth UI while dragging
    {
    ...
    if (completed) {
        ...
        // Backend may clamp or reorder due to constraints, so safest:
        reload();
    }
}
